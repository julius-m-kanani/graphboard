...
export const toolCopy = {
  ...
  square: ['Square', 'Drag to set the size — sides stay equal.'], rectangle: ['Rectangle', 'Drag to set the width and height.'], triangle: ['Equilateral triangle', 'Drag to set the size of an equilateral triangle.'], pentagon: ['Pentagon', 'Drag to set the size of a regular pentagon.'], hexagon: ['Hexagon', 'Drag to set the size of a regular hexagon.'], star: ['Star', 'Drag to set the size of a five-pointed star.']
};
...
export const SHAPES = ['square', 'rectangle', 'triangle', 'pentagon', 'hexagon', 'star'];
...
export function closestPointOnShape(w, a) {
  const pts = shapeVertices(a.type, a.from, a.to); let best = null, bestD = Infinity;
...
export function shapeDistance(w, a) { const q = closestPointOnShape(w, a); return q ? pointDistance(w, q) : Infinity; }
...
export function updateReadout(p) { const w = screenToWorld(p); let extra = ''; const a = state.drawing; if (a && (a.type === 'line' || a.type === 'ruler' || a.type === 'set-square')) { const len = pointDistance(a.from, a.to); extra = `&nbsp;&nbsp;from (${pretty(a.from.x)}, ${pretty(a.from.y)}) → (${pretty(a.to.x)}, ${pretty(a.to.y)}) · ${pretty(len)} u`; } else if (a && a.type === 'ray') { const len = pointDistance(a.from, a.to); extra = `&nbsp;&nbsp;from (${pretty(a.from.x)}, ${pretty(a.from.y)}) to (${pretty(a.to.x)}, ${pretty(a.to.y)}) · ${pretty(len)} u`; } else if (a && SHAPES.includes(a.type)) { extra = `&nbsp;&nbsp;width ${pretty(Math.abs(a.to.x - a.from.x))} × height ${pretty(Math.abs(a.to.y - a.from.y))}`; } else { let point = null; for (const act of state.actions) if (act.type === 'point' && pointDistance(act.at, w) < .55) { point = act; break; } if (point) { extra = `&nbsp;&nbsp;${point.label ? point.label + ' ' : ''}(${pretty(point.at.x)}, ${pretty(point.at.y)})`; } else { const m = pointOnMark(w); if (m) extra = `&nbsp;&nbsp;on line (${pretty(m.x)}, ${pretty(m.y)})`; } } $('#coordinate-readout').innerHTML = `x: ${pretty(w.x)}&nbsp;&nbsp; y: ${pretty(w.y)}${extra}`; }

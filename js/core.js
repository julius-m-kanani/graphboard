...
export function pointOnMark(w) { let best = null, bestD = Infinity; state.actions.forEach(a => { let d = Infinity, p = null; if (a.type === 'line' || a.type === 'ruler' || a.type === 'set-square') { d = segmentDistance(w, a.from, a.to); p = closestPointOnSegment(w, a.from, a.to); } else if (a.type === 'angle') { d = Math.min(pointDistance(w, a.center), segmentDistance(w, a.center, a.end)); p = pointDistance(w, a.center) <= segmentDistance(w, a.center, a.end) ? { x: a.center.x, y: a.center.y } : closestPointOnSegment(w, a.center, a.end); } else if (a.type === 'circle') { d = Math.abs(pointDistance(w, a.center) - a.radius); const r = a.radius, dist = pointDistance(w, a.center) || 1; p = { x: a.center.x + (w.x - a.center.x) * r / dist, y: a.center.y + (w.y - a.center.y) * r / dist }; } if (p && d < bestD) { bestD = d; best = p; } }); return best && bestD < .3 ? best : null; }
export function updateReadout(p) { const w = screenToWorld(p); let extra = ''; const a = state.drawing; if (a && (a.type === 'line' || a.type === 'ruler' || a.type === 'set-square')) { const len = pointDistance(a.from, a.to); extra = `&nbsp;&nbsp;from (${pretty(a.from.x)}, ${pretty(a.from.y)}) → (${pretty(a.to.x)}, ${pretty(a.to.y)}) · ${pretty(len)} u`; } else { const m = pointOnMark(w); if (m) extra = `&nbsp;&nbsp;on line (${pretty(m.x)}, ${pretty(m.y)})`; } $('#coordinate-readout').innerHTML = `x: ${pretty(w.x)}&nbsp;&nbsp; y: ${pretty(w.y)}${extra}`; }

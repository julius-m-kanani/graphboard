...
export function updateReadout(p) { const w = screenToWorld(p); let extra = ''; const a = state.drawing; if (a && (a.type === 'line' || a.type === 'ruler' || a.type === 'set-square')) { const len = pointDistance(a.from, a.to); extra = `&nbsp;&nbsp;from (${pretty(a.from.x)}, ${pretty(a.from.y)}) → (${pretty(a.to.x)}, ${pretty(a.to.y)}) · ${pretty(len)} u`; } else if (a && (a.type === 'ray' || a.type === 'dotted')) { const len = pointDistance(a.from, a.to); extra = `&nbsp;&nbsp;from (${pretty(a.from.x)}, ${pretty(a.from.y)}) to (${pretty(a.to.x)}, ${pretty(a.to.y)}) · ${pretty(len)} u`; } else if (a && SHAPES.includes(a.type)) { const bw = Math.abs(a.to.x - a.from.x), bh = Math.abs(a.to.y - a.from.y); extra = a.type === 'circle' ? `&nbsp;&nbsp;radius ${pretty(bw / 2)} u · diameter ${pretty(bw)} u` : a.type === 'ellipse' ? `&nbsp;&nbsp;rx ${pretty(bw / 2)} × ry ${pretty(bh / 2)}` : `&nbsp;&nbsp;width ${pretty(bw)} × height ${pretty(bh)}`; } else { let point = null; for (const act of state.actions) if (act.type === 'point' && pointDistance(act.at, w) < .55) { point = act; break; } if (point) { extra = `&nbsp;&nbsp;${point.label ? point.label + ' ' : ''}(${pretty(point.at.x)}, ${pretty(point.at.y)})`; } else { const m = pointOnMark(w); if (m) extra = `&nbsp;&nbsp;on line (${pretty(m.x)}, ${pretty(m.y)})`; } } $('#coordinate-readout').innerHTML = `x: ${pretty(w.x)}&nbsp;&nbsp; y: ${pretty(w.y)}${extra}`; }
export function setTool(tool) { state.tool = tool; $$('.tool').forEach(el => el.classList.toggle('active', el.dataset.tool === tool)); const sel = $('#shape-select'); if (sel) sel.value = SHAPES.includes(tool) ? tool : ''; const [name, copy] = toolCopy[tool]; setToolTip(name, copy); canvas.style.cursor = canvasCursor(); render(); }

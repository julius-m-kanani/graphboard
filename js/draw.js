...
function drawAction(a, preview = false) {
  ...
  if (a.type === 'ray') {
    ...
    if (len >= 2) { const ux = dx / len, uy = dy / len, end = rayClip(p1, { x: ux, y: uy }, width, height) || { x: p1.x + ux * (width + height), y: p1.y + uy * (width + height) }; ctx.lineCap = 'butt'; ctx.setLineDash([2, 4]); ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(p1.x, p1.y); ctx.lineTo(end.x, end.y); ctx.stroke(); ctx.setLineDash([]); ctx.lineCap = 'round'; const ex = end.x, ey = end.y, tip = 9; ctx.beginPath(); ctx.moveTo(ex, ey); ctx.lineTo(ex - ux * tip + uy * 4, ey - uy * tip - ux * 4); ctx.lineTo(ex - ux * tip - uy * 4, ey - uy * tip + ux * 4); ctx.closePath(); ctx.fill(); ctx.beginPath(); ctx.arc(p1.x, p1.y, 3.2, 0, Math.PI * 2); ctx.fill(); }
  }

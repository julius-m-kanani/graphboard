...
function eraseNear(w) {
  ...
  state.actions.forEach((a, i) => { let d = Infinity; if (a.type === 'point') d = pointDistance(a.at, w); else if (a.type === 'circle') d = Math.abs(pointDistance(a.center, w) - a.radius); else if (a.type === 'pencil') d = Math.min(...a.points.map(p => pointDistance(p, w))); else if (a.type === 'compass') d = polylineDistance(w, a.points); else if (a.type === 'line' || a.type === 'ruler' || a.type === 'set-square' || a.type === 'ray' || a.type === 'dotted') d = segmentDistance(w, a.from, a.to); else if (SHAPES.includes(a.type)) d = shapeDistance(w, a); else if (a.type === 'angle') d = Math.min(pointDistance(w, a.center), segmentDistance(w, a.center, a.end)); if (d < dist) { dist = d; best = i; } });
...
function pickAction(w) {
  ...
  state.actions.forEach((a, i) => { let d = Infinity; if (a.type === 'point') d = pointDistance(a.at, w); else if (a.type === 'circle') d = Math.min(pointDistance(a.center, w), Math.abs(pointDistance(a.center, w) - a.radius)); else if (a.type === 'pencil') d = Math.min(...a.points.map(p => pointDistance(p, w))); else if (a.type === 'compass') d = polylineDistance(w, a.points); else if (a.type === 'line' || a.type === 'ruler' || a.type === 'set-square' || a.type === 'ray' || a.type === 'dotted') d = segmentDistance(w, a.from, a.to); else if (SHAPES.includes(a.type)) d = shapeDistance(w, a); else if (a.type === 'angle') d = Math.min(pointDistance(w, a.center), segmentDistance(w, a.center, a.end)); if (d < bestD) { bestD = d; best = i; } }); return best >= 0 && bestD < .55 ? state.actions[best] : null;
}
function applyMove(a, dx, dy) {
  ...
  if (a.type === 'line' || a.type === 'ruler' || a.type === 'set-square' || a.type === 'ray' || a.type === 'dotted' || SHAPES.includes(a.type)) { a.from.x += dx; a.from.y += dy; a.to.x += dx; a.to.y += dy; return; }
...
function endDraw(e) {
  ...
  if ((a.type === 'line' || a.type === 'ruler' || a.type === 'set-square' || a.type === 'ray' || a.type === 'dotted' || SHAPES.includes(a.type)) && pointDistance(a.from, a.to) < .1) { render(); return; }
